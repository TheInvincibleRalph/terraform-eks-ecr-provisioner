...
Before any infrastructure is touched, the pipeline runs a formatting check and validates the syntax. It then runs a `terraform plan` to verify exactly what changes will be made to the AWS environment. This ensures broken code never reaches the cloud.
